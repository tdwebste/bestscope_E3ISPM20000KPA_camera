...
LONG WINAPI  AppWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(msg)
    {
        ...
        case WM_INITMENU:
			...
			for (UINT i = MENU_VDEVICE00; i < MENU_VDEVICE39; ++i)
            // can't select a new capture device when capturing
				EnableMenuItem((HMENU)wParam, i, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
...
void AddDevicesToMenu()
{
    ...
    while(hr = pEm->Next(1, &pM, &cFetched), hr==S_OK)
    {
        ...
        if(SUCCEEDED(hr))
        {
            ...
            if(hr == NOERROR)
            {
                AppendMenu(hMenuSub, MF_STRING, MENU_VDEVICE00 + uIndex,
                    var.bstrVal);

                if(gcap.pmVideo != 0 && (S_OK == gcap.pmVideo->IsEqual(pM)))
                    bCheck = TRUE;

                CheckMenuItem(hMenuSub,  MENU_VDEVICE00 + uIndex,
                    (bCheck ? MF_CHECKED : MF_UNCHECKED));
                EnableMenuItem(hMenuSub, MENU_VDEVICE00 + uIndex,
...
LONG PASCAL AppCommand(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(id)
    {
		...
		case MENU_VDEVICE38:
		case MENU_VDEVICE39:
            ChooseDevices(gcap.rgpmVideoMenu[id - MENU_VDEVICE00], gcap.pmAudio);
            break;
