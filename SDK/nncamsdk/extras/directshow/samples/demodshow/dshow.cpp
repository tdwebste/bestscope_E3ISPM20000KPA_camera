...
void TDshowContext::MakeMenuOptions(BOOL bAdd, HMENU hMenuSub1, UINT pos, UINT times, UINT cmdID)
{
    HRESULT hr;
...
HRESULT TDshowContext::InitCapFilters()
{
    HRESULT hr = S_OK;
...
BOOL TDshowContext::RenderStillPin()
{
		...
		return FALSE;
	{
		...
		if (pGrabber)
		{
			...
			pGrabber->SetCallback(&m_SampleGrabberCBImpl, 0);
		}
	}
...
HRESULT TDshowContext::start(HMENU hMenuSub1, UINT pos, UINT times, UINT cmdID)
{
	...
	MakeMenuOptions(TRUE, hMenuSub1, pos, times, cmdID);
...
BOOL TDshowContext::snapshot(IBaseFilter* pSampleGrabber, const wchar_t* filename)
{
	...
	{
		if (m_SampleGrabberCBImpl.bCapturing())
			return FALSE;

		m_SampleGrabberCBImpl.start(filename, spGrabber);
...
STDMETHODIMP SampleGrabberCBImpl::SampleCB(double SampleTime, IMediaSample* pSample)
{
	if (pSample)
	{
		...
		{
			AM_MEDIA_TYPE mt = { 0 };
			HRESULT hr = pISampleGrabber_->GetConnectedMediaType(&mt);
			if (SUCCEEDED(hr))
			{
				// Examine the format block.
				if ((mt.formattype == FORMAT_VideoInfo) && 
				...
				{
					VIDEOINFOHEADER* pVih = (VIDEOINFOHEADER*)mt.pbFormat;
					savebitmap(&(pVih->bmiHeader), (const char*)pBuffer, pSample->GetActualDataLength(), filename_.c_str());
				}
				FreeMediaType(mt);
			}
		}
