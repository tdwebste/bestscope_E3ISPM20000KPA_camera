...
#End If
    ...
    Public Enum eFLAG As Long
        ...
        FLAG_HEAT = &H8000000000                  ' heat to prevent fogging up
        FLAG_LOW_NOISE = &H10000000000            ' low noise mode
    End Enum
    ...
    Public Enum eOPTION As UInteger
       ...
       OPTION_DFC                    = &H1d       ' dark field correction
        ...
        OPTION_MAX_PRECISE_FRAMERATE = &H2c       ' precise frame rate maximum value in 0.1 fps, such as 115 means 11.5 fps. E_NOTIMPL means not supported
        ...
        OPTION_FRAME_DEQUE_LENGTH    = &H31       ' frame buffer deque length, range: [2, 1024], default: 3
        OPTION_MIN_PRECISE_FRAMERATE = &H32       ' precise frame rate minimum value in 0.1 fps, such as 15 means 1.5 fps
        OPTION_SEQUENCER_ONOFF       = &H33       ' sequencer trigger: on/off
        OPTION_SEQUENCER_NUMBER      = &H34       ' sequencer trigger: number, range = [1, 255]
        OPTION_SEQUENCER_EXPOTIME    = &H01000000 ' sequencer trigger: exposure time, iOption = TOUPCAM_OPTION_SEQUENCER_EXPOTIME | index, iValue = exposure time
        ...
        OPTION_SEQUENCER_EXPOGAIN    = &H02000000 ' sequencer trigger: exposure gain, iOption = TOUPCAM_OPTION_SEQUENCER_EXPOGAIN | index, iValue = gain
        OPTION_DENOISE               = &H35       ' denoise, strength range: [0, 100], 0 means disable
        OPTION_HEAT_MAX              = &H36       ' maximum level: heat to prevent fogging up
        OPTION_HEAT                  = &H37       ' heat to prevent fogging up
        OPTION_LOW_NOISE             = &H38       ' Low noise mode: 1 => enable
    End Enum
...
#End If
    ...
    <DllImport("toupcam.dll", ExactSpelling:=True, CallingConvention:=CallingConvention.StdCall)>
    Private Shared Function Toupcam_Replug(<MarshalAs(UnmanagedType.LPWStr)> id As String) As Integer
    End Function
...
#End If
    ...
    Public Shared Function [EnumV2]() As DeviceV2()
        Dim p As IntPtr = Marshal.AllocHGlobal(512 * 16)
        Dim ti As IntPtr = p
        Dim cnt As UInteger = Toupcam_EnumV2(p)
...
#End If
    ...
    Public Function get_FinalSize(ByRef nWidth As Integer, ByRef nHeight As Integer) As Boolean
        ...
        If _handle Is Nothing OrElse _handle.IsInvalid OrElse _handle.IsClosed Then
            Return False
        End If
        Return (Toupcam_get_FinalSize(_handle, nWidth, nHeight) >= 0)
    End Function
    ...
    Public Function get_RealTime(ByRef val As Integer) As Boolean
        ...
        If _handle Is Nothing OrElse _handle.IsInvalid OrElse _handle.IsClosed Then
            Return False
        End If
        Return (Toupcam_get_RealTime(_handle, val) >= 0)
    End Function
    ...
    Public Function IoControl(index As UInteger, eType As eIoControType, inVal As Integer, ByRef outVal As UInteger) As Boolean
        If _handle Is Nothing OrElse _handle.IsInvalid OrElse _handle.IsClosed Then
            Return False
        End If
